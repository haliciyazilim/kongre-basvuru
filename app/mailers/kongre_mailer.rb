class KongreMailer < ActionMailer::Base
  ...
  def attendance_info_mail(email, card_number, workshops)
    ...
    mail(to: email, subject: 'ZEKA VE EĞİTİM ZİRVESİ')
  end
  ...
  def send_coupon_mail(email, code)
    ...
    mail(:to => email, :subject => 'Zeka ve Eğitim Zirvesi Katılım Bedeli İndirim Kuponu')
  end
  ...
  def daily_info_mail(email)
    ...
    mail(to: email, subject: 'Zeka ve Eğitim Zirvesi - Günlük Bilgilendirme')
  end
end
